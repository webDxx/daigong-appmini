...
const Reports: React.FC<{ data: AppData }> = ({ data }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div>
        <h2 className="text-3xl font-black text-slate-800 tracking-tight">统计中心</h2>
        <p className="text-slate-500 mt-1">深度剖析生产效能与各渠道营收构成。</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
          <div className="flex items-center gap-2 mb-8">
            ...
            <h3 className="font-bold text-slate-800 text-lg">营收渠道构成</h3>
          </div>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
          <div className="flex items-center gap-2 mb-8">
            ...
            <h3 className="font-bold text-slate-800 text-lg">工人产量贡献</h3>
          </div>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex items-center gap-2">
            ...
            <h3 className="font-bold text-slate-800">工人工作量及产能统计</h3>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/30 text-slate-400 text-[10px] font-black uppercase tracking-widest border-b border-slate-100">
                  <th className="px-8 py-5">工人姓名</th>
                  <th className="px-8 py-5 text-center">累计订单</th>
                  <th className="px-8 py-5 text-center">总生产件数</th>
                  <th className="px-8 py-5 text-center">当前进度</th>
                  <th className="px-8 py-5 text-right">预估工费支出</th>
                </tr>
              </thead>
                ...
                {workerPerformance.map(w => {
                   ...
                   return (
                    <tr key={w.name} className="hover:bg-slate-50 transition-colors">
                      <td className="px-8 py-5 font-bold text-slate-800">{w.name}</td>
                      <td className="px-8 py-5 text-center text-slate-600 font-medium">{w.orderCount} 笔</td>
                      <td className="px-8 py-5 text-center text-slate-900 font-black">{w.total} <span className="text-slate-400 text-[10px] font-normal">条</span></td>
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[10px] font-black text-slate-400">{w.completionRate}%</span>
                        </div>
                      </td>
        ...
        <div className="space-y-8">
            <div className="bg-gradient-to-br from-slate-900 to-indigo-950 p-10 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:scale-125 transition-transform duration-500">
                    <Landmark size={120} />
                </div>
                <div className="relative z-10 space-y-8">
                    <div>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">核心财务漏斗</p>
                        <h4 className="text-4xl font-black">¥{financialSummary.totalIncome.toLocaleString()}</h4>
                        <p className="text-xs text-slate-400 mt-1">累计总销售收入</p>
                    </div>
                    ...
                    <div className="pt-8 border-t border-white/10">
                        <div className="flex items-center justify-between">
                            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">总体毛利率估算</span>
