...
import { Package, Truck, DollarSign, AlertTriangle, TrendingUp, CreditCard, Plus, Users, BarChart2, Tv } from "lucide-react";
import { differenceInDays, isBefore } from "date-fns";

const StatCard = ({ title, value, unit, icon: Icon, colorClass, subText, to }: any) => (
	<Link
		to={to}
		className="bg-white p-2 lg:p-3 rounded-lg border border-slate-100 shadow-sm hover:shadow-md hover:border-blue-200 transition-all active:scale-95 cursor-pointer"
	>
...
const Dashboard: React.FC<{ data: AppData }> = ({ data }) => {
	const stats = useMemo(() => {
		...
		const totalPaidExpenditure =
			data.orders.reduce((sum, o) => sum + (o.paid_amount || 0), 0) + data.transfers.reduce((sum, t) => sum + t.amount, 0);
	...
	const bankCardStats = useMemo(() => {
		...
		return cards.map((card) => {
			// 收入：销售收入
			const income = data.incomes.filter((i) => i.bank_card === card).reduce((sum, i) => sum + i.amount, 0);

			// 支出：订单已付工费 + 其他支出
			const orderExpenditure = data.orders.filter((o) => o.bank_card === card).reduce((sum, o) => sum + (o.paid_amount || 0), 0);

			const otherExpenditure = data.expenses.filter((e) => e.bank_card === card).reduce((sum, e) => sum + e.amount, 0);
	...
	const workerStats = useMemo(() => {
		return data.workers
			.slice(0, 5)
			.map((w) => {
				const totalQty = data.orders.filter((o) => o.worker_id === w.id).reduce((sum, o) => sum + o.quantity, 0);
		...
		<div className="space-y-2 lg:space-y-4 pb-4">
			...
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-3">
				...
				<StatCard
					...
					to="/orders"
				/>
				<StatCard title="签约工人" value={data.workers.length} unit="位" icon={Users} colorClass="bg-indigo-600" to="/workers" />
				<StatCard
					title="主播管理"
					...
					subText={`直播记录: ${data.liveSessions.length}场`}
					to="/anchors"
				/>
				<StatCard
					title="主播工资"
			...
			<div className="bg-white rounded-lg border border-slate-100 shadow-sm overflow-hidden">
				...
				<div className="divide-y divide-slate-50">
					{delayedOrders.length > 0 ? (
								...
								<div className="flex items-center gap-2">
									...
									<div>
										<p className="font-bold text-slate-800 text-[10px]">
											{order.workerName} <span className="text-slate-400 font-normal">({order.quantity}条)</span>
										</p>
						...
						<div className="p-6 text-center text-slate-300 text-[9px] font-bold uppercase tracking-widest">暂无延期</div>
					)}
				</div>
			</div>
							...
							return (
								<div key={idx} className="flex items-center gap-2">
									...
									<div className="flex-1 bg-slate-100 h-4 rounded-full overflow-hidden">
										<div className="bg-indigo-500 h-full rounded-full transition-all" style={{ width: `${widthPercent}%` }}></div>
									</div>
